...
impl Core {
    pub async fn new() -> Self {
        ...
        let file_handler: file::FileHandler = file::FileHandler::new(&env.storage_directory);
        let signature_handler: signature::SignatureHandler = signature::SignatureHandler::new(
            &env.database_url,
            &env.database_name,
            &env.database_collection
        ).await;
        let files_status: HashMap<String, String> =  HashMap::new();
    ...
    async fn verify_files(&mut self) {
        info!("Verifying all stored files.");
        let files = fs::read_dir(&self.file_handler.storage_dir).expect("Failed to list directory");
        ...
        for file in files {
            ...
            match self.signature_handler.load_signature(&file_name).await {
                ...
                None => {
                    self.signature_handler.save_signature(&file_name, &current_signature).await.expect("Failed to save signature");
